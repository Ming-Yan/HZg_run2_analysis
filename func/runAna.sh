#!bin/bash
read -p "start job : end job" sjob ejob
read -p " start/end file " min max
read -p "HOW many file one time? " nf
read -p "is Data/MC? " datamc
interval=$(($ejob - $sjob + 1))
if [ "${datamc}" == "data" ]; then
    for ((job = 0 ; job < ${interval} ; job++));
    do
	fmin=$(($min + $job * $nf))
	fmax=$(($min + $job* $nf + $nf))
	outjob=$(($job + $sjob))
	...
	for ifile in (seq 1 9)
	cat>runData${outjob}_${ifile}.C<<EOF
  ...
  string sample[18] = {
    ...
    "Mu_Run2016H_PRv3",*/
		  ...
		  for (int i = ${fmin};i < ${fmax} ; i++){
			sprintf(inpath,"/eos/cms/store/group/phys_smp/ggNtuples/13TeV/data/V08_00_26_07/job_Double%s/*/*/*/0000/ggtree_data_${ifile}*.root",sample[i].c_str());
			...
			TString outname = "Data_VBFMVA_update0811_0000_${ifile}x";
...
EOF
	nohup root -l -b -q runData${outjob}_${ifile}.C &>log/data_${outjob}${ifile}.txt &
	sleep 10s
	echo "finish Data ${outjob} : from ${min} to ${max}" 
    done
else 
    for ((job = 0 ; job < ${interval} ; job++));
    do
...
EOF
	nohup root -l -b -q runAna_v${outjob}.C &>mc_${outjob}.txt &
	sleep 20s
	echo "finish MC ${outjob} : from ${min} to ${max}" 
    done
fi
