...
void mana2(const char* inpath, TString outpath = "minitree.root", Int_t channel=0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, int isGH = 0, Int_t processID=0, Int_t set = 0, Int_t member = 0)
//void mana(Int_t channel = 0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, Int_t processID=0)
{
  ...
  //           data.Print();
  // read in SFs and
  TFile *fe_trg1 = new TFile("external/Trigger_Leg1_Ele23_Moriond17_SFs.root");
  TFile *fe_trg2 = new TFile("external/Trigger_Leg2_Ele12_Moriond17_SFs.root");
  ...
  TFile *fm_trg1_0to09 = new TFile("external/Trigger_Leg1_Mu17_Eta0to09_Moriond17_SFs.root");
  TFile *fm_trg1_09to24 = new TFile("external/Trigger_Leg1_Mu17_Eta09to24_Moriond17_SFs.root");
  TFile *fm_trg2_0to09 = new TFile("external/Trigger_Leg2_Mu8_Eta0to09_Moriond17_SFs.root");
  TFile *fm_trg2_09to24 = new TFile("external/Trigger_Leg2_Mu8_Eta09to24_Moriond17_SFs.root");
  ...
  TFile *fm_IsoLowPt = new TFile("external/Muon_Iso_pt10to20_Moriond17_SFs.root");
  TFile *fm_IDLowPt = new TFile("external/Muon_ID_pt10to20_Moriond17_SFs.root");
  ...
  TH1F              *hm_trg1_0to09  = (TH1F*) fm_trg1_0to09->Get("h1");
  TH1F              *hm_trg1_09to24  = (TH1F*) fm_trg1_09to24->Get("h1");
  TH1F              *hm_trg2_0to09  = (TH1F*) fm_trg2_0to09->Get("h1");
  TH1F              *hm_trg2_09to24  = (TH1F*) fm_trg2_09to24->Get("h1");
  TH2F              *hm_IDBtoF  = (TH2F*) fm_IDBtoF->Get("MC_NUM_TightID_DEN_genTracks_PAR_pt_eta/abseta_pt_ratio");
  TH2F              *hm_IsoLowPt = (TH2F*) fm_IsoLowPt->Get("hM_merge");
  TH2F              *hm_IDLowPt = (TH2F*) fm_IDLowPt->Get("hM_merge");
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++)
      ...
      for (Int_t i = 0; i < nMu ; i++)
	{
	  ...
	  if (data.HasMC()){
	    ...
	    if (countmc > 0) SF = rc.kScaleFromGenMC(muCharge[i], muPt[i], muEta[i], muPhi[i], muTrkLayers[i], mcMu.Pt(), rand1, set, member);
	    if (countmc == 0) SF = rc.kScaleAndSmearMC(muCharge[i], muPt[i], muEta[i], muPhi[i], muTrkLayers[i], rand1, rand2, set, member);
	  }
      ...
      if(isVBF!=1&&isLep!=1)
	{
	  ...
	  if(cat<6||cat>9) sfGSF_[2] = sfeID_[2]  = sfmuID_[2] = sfmuIso_[2] = sfmuTrk_[2] = 1.; //set the SFs for no lepton tag cat
		  ...
		  Float_t tmpMuPt = (alepPt< 120) ? alepPt : 119;
		  ...
		  if (alepPt >= 20)
		      ...
		      unmuID_[2] =  hm_IDLowPt->GetBinError(hm_IDLowPt->FindBin(tmpMuPt, fabs(alepEta)));
	  ...
	  for(Int_t i = 0 ; i <2 ; i++)
	      ...
	      if(fabs(lepEta_[i])<=0.9) 
		{
		  if(i==0) 
		    ...
		    {
		      sfmutrg_[1] = hm_trg2_0to09->GetBinContent(hm_trg2_0to09->FindBin(tmpMuPt));
		      unmutrg_[1] = hm_trg2_0to09->GetBinError(hm_trg2_0to09->FindBin(tmpMuPt));
		    }
		}
	      else
		{
		  if(i==0)
		    {
		      sfmutrg_[0] = hm_trg1_09to24->GetBinContent(hm_trg1_09to24->FindBin(tmpMuPt));
		      unmutrg_[0] = hm_trg1_09to24->GetBinError(hm_trg1_09to24->FindBin(tmpMuPt));
		    }
		  else
		    {
		      sfmutrg_[1] = hm_trg2_09to24->GetBinContent(hm_trg2_09to24->FindBin(tmpMuPt));
		      unmutrg_[1] = hm_trg2_09to24->GetBinError(hm_trg2_09to24->FindBin(tmpMuPt));
		    }
		}
		  ...
		  unmuIso_[i] = (hm_IsoBtoF->GetBinError(hm_IsoBtoF->FindBin(fabs(lepEta_[i]), tmpMuPt))*19.715+
                ...
                {
		  sfmuID_[i] = hm_IDLowPt->GetBinContent(hm_IDLowPt->FindBin(tmpMuPt, fabs(lepEta_[i])));
		  unmuID_[i] =  hm_IDLowPt->GetBinError(hm_IDLowPt->FindBin(tmpMuPt, fabs(lepEta_[i])));
...
*/
  ...
  delete fm_trg1_09to24;
  delete fm_trg2_0to09;
  delete fm_trg2_09to24;
  delete fm_Trk;
  delete fm_IDBtoF;
  delete fm_IsoBtoF;
  delete fm_IDGH;
