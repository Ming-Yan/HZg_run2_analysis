...
//#include "plugin/untuplizer.h"//--for data
//#include "plugin/dR.h"
...
//void mana(Int_t channel = 0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, Int_t processID=0)
{
  ...
  */
  ...
  Float_t Zpt_, Vtz_,Vtx_,Vty_,mcVtz_,mcVtx_,mcVty_,d0_[2],dz_[2],SIP_[2];//temporary stored due to study
  ...
  ///studying VARS///
  outtreeZg_->Branch("d0",        d0_,        "d0[2]/F");
  outtreeZg_->Branch("dz",        dz_,        "dz[2]/F");
  outtreeZg_->Branch("SIP",       SIP_,       "SIP[2]/F");
  outtreeZg_->Branch("Zpt",       &Zpt_,      "Zpt/F");
  outtreeZg_->Branch("Vtx",       &Vtx_,      "Vtx/F");
  outtreeZg_->Branch("Vty",       &Vty_,      "Vty/F");
  outtreeZg_->Branch("Vtz",       &Vtz_,      "Vtz/F");
  outtreeZg_->Branch("mcVtx",     &mcVtx_,    "mcVtx/F");
  outtreeZg_->Branch("mcVty",     &mcVty_,    "mcVty/F");
  outtreeZg_->Branch("mcVtz",     &mcVtz_,    "mcVtz/F");
  ///studying VARS--end///
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++)
    {
      ...
      isVBF = -1;
      cat = -1;
      lepmatch[0] = lepmatch[1] = phomatch = totmatch = 0;
      bool isSig = false;
      data.GetEntry(ev);
      if (ev % 5000000 == 0)
        {
      ...
      //trigger end
      //lepton
      //      float *vtz = data.GetPtrFloat("vtz");
      float vtx = data.GetFloat("vtx");
      float vty = data.GetFloat("vty");
      float vtz = data.GetFloat("vtz");
      ...
      */        
	  ...
	  for (Int_t j = 0 ; j < nMC; ++j)
            {
	      if (((mcStatusFlag[j] >> 0) & 1) == 0 || ((mcStatusFlag[j] >> 1) & 1) == 0) continue;
	      if(fabs(mcEta[j])>2.5) continue;
	      if(mcPt[j]<15) continue;
	      if (fabs(mcPID[j]) != 11) continue;
	      if (mcGMomPID[j] != 25) continue;
	      if (mcMomPID[j] !=23) continue;
	      mcLepPt_[nmcLep] = mcPt[j];	      
	      if(event_==299654)cout<<mcLepPt_[nmcLep]<<endl;
	      mcLepEta_[nmcLep] = mcEta[j];
	      mcLepPhi_[nmcLep] = mcPhi[j];
	      mcVtx_ = mcVtx[j];	      
	      mcVty_ = mcVty[j];
	      mcVtz_ = mcVtz[j];
	      isSig = true;
	  ...
	  for (Int_t j=0; j< 2; ++j)
	      ...
	      d0_[j] = eleD0[ZeeID[j]];
              dz_[j] = eleDz[ZeeID[j]];
              SIP_[j] = eleSIP[ZeeID[j]];
              //Vtz_[j] = vtz[ZeeID[j]];
	      nLep++;
	      if(event_==299654)cout<<"before"<<lepmatch[j]<<endl;
	      if(isSig==true&&mclep[j].DeltaR(lep[j])<0.1) lepmatch[j] = 1;
	      if(event_==299654)cout<<lepmatch[j]<<endl;
            }
	  Vtx_ = vtx;
	  Vty_ = vty;
	  Vtz_ = vtz;
	  ...
	  for (Int_t j=0; j<nMC; ++j)
            {
	      if (((mcStatusFlag[j] >> 0) & 1) == 0 || ((mcStatusFlag[j] >> 1) & 1) == 0) continue;
	      if(mcPt[j]<10) continue;
	      if(fabs(mcEta[j])>2.4) continue;
	  ...
	  for (Int_t i=0; i< 2; ++i)
	      ...
	      dz_[i] = muDz[Zmm[i]];
	      SIP_[i] = muSIP[Zmm[i]];
	      lep[i].SetPtEtaPhiM(muPt[Zmm[i]], muEta[Zmm[i]], muPhi[Zmm[i]], 105.7*0.001);
	      nLep++;
	      isPF[i] = (muType[Zmm[i]] >> 5 & 1);
	      if(isSig==true&&mclep[i].DeltaR(lep[i])<0.1) lepmatch[i] = 1;
            }
	  Vtx_ = vtx;
	  Vty_ = vty;
	  Vtz_ = vtz;
      ...
      if (nLep < 2) continue;
      zll   = lep[0] + lep[1];
      if(isSig==true)mcll  = mclep[0] + mclep[1];
      if(isSig==true)mcmll_ = mcll.M();
      ...
      for (Int_t i=0; i<nMC; ++i)
	{
      ...
      if(isSig==true)mcllg = mcll + mcpho;
      if(isSig==true)mcmllg_ = mcllg.M();
  ...
  outtreeMC_->Write("",TObject::kOverwrite);
