...
#include "/afs/cern.ch/user/m/milee/CMSSW_7_4_7/src/flashggFinalFit/Signal/interface/WSTFileWrapper.h"
...
void OptionParser(int argc, char *argv[]){
  ...
  desc1.add_options()
    ...
    ("procs,p", po::value<string>(&procString_)->default_value("ggF,VBF,WH,ZH,ttH"),          "Processes")
    ("channel,c", po::value<string>(&channelString_)->default_value("ele,mu"), 				"electron or muon channel")
    ("recursive",									  											"Reursive fraction")
    ...
    ("flashCats,f", po::value<string>(&flashggCatsStr_)->default_value("cat1,cat2,cat3,cat4,cat5,cat501,cat502,cat6789"),       	"HZg category names to consider")
...
double getMyNLL(RooRealVar *var, RooAbsPdf *pdf, RooDataHist *data){
  ...
  for (int i=0; i<data->numEntries(); i++){
    double binCenter = data->get(i)->getRealValue("CMS_hzg_mass");
...
int main(int argc, char *argv[]){
  ...
  // int   nBins    =  int(rangeHigh -rangeLow); 
  ...
  split(chs,channelString_,boost::is_any_of(","));
  ...
  for (unsigned int c=0; c<chs.size(); c++)
    {
      vector<int> colors;
	...
	if(verbose_) std::cout << "[INFO] on proc " << procs[p] <<  " start looping through nCats " << ncats_ << " channels "<< chs[c]<< " to book RooPlots " <<  std::endl;
	for (int cat=0; cat<ncats_; cat++){
	  ...
	  plotRV->SetTitle(
			   Form("%s_%s_%s_RV",chs[c].c_str(),procs[p].c_str(),flashggCats_[cat].c_str()));
	...
	for (unsigned int p=0; p<procs.size(); p++){
	  ...
	  if (verbose_) std::cout << "[INFO] got roorealvars from ws ? mass " << mass << std::endl;
      
	  // access dataset and immediately reduce it!
	  if (isFlashgg_){
	    RooDataSet *data0   = (RooDataSet*)inWS->data(
							  Form("%s_%s_%d_%s",ch.c_str(),proc.c_str(),mass_,flashggCats_[cat].c_str()));
	    if(verbose_)std::cout<<"[INFO] get data "<<ch<<"_"<<proc<<"_"<<mass_<<"_"<<flashggCats_[cat]<<endl;
	    // dataRV   = (RooDataSet*)inWS->data(
	    if(verbose_) {
	    ...
	    for (unsigned int i=0 ; i < dataRV->numEntries() ; i++){
	      mass->setVal(dataRV->get(i)->getRealValue("CMS_hzg_mass"));
	      weight0->setVal(dataRV->weight() ); // <--- is this correct?
		...
		<< Form("%s_%s_%d_13TeV_%s",ch.c_str(),proc.c_str(),mass_,flashggCats_[cat].c_str()) <<endl;
	      // << "? "<< data<<std::endl;
	    }
	  ...
	  while (prob_old<rv_prob_limit && order <5){ 
	    ...
	    prob_old = TMath::Prob(chi2,diffInDof);
	  ...
	  if (prob <rv_prob_limit){
		...
		std::cout<<"prob<rv_prob"<<std::endl;
	      }
	    }
	  }else {
	  ...
	  choices.insert(pair<string,int >(Form("%s %d",proc.c_str(),cat),rvChoice));
	...
	//  ^ NB at the moment the combiend plot is not terribly useful, so ignore it for now.
      ...
      //finally, print out recommended options. This is kinda useless right now..
      ...
      for (map<string,int>::iterator it=choices.begin();
	   it!=choices.end();
	   it++){
      
	//print to screen
	cout << "[INFO] \t"<<" "<< it->first 
	     << " this is second" << it->second<< endl;
