...
void make_bias_hists(string filename, string outfilename, string cat, float expmean=0., int meanBins=30, float meanL=-3., float meanH=3., int resBins=50, float resL=0.5, float resH=1.){
  
  string treename = Form("muTree");
  ...
  vector<double> *muPaulErrLow=0;
  vector<double> *muPaulErrHigh=0;
  vector<double> *muChi2ErrLow=0;
  vector<double> *muChi2ErrHigh=0;
  vector<double> *muAICErrLow=0;
  vector<double> *muAICErrHigh=0;
  ...
  chain->SetBranchAddress("muPaulErrLow",&muPaulErrLow);
  chain->SetBranchAddress("muPaulErrHigh",&muPaulErrHigh);
  chain->SetBranchAddress("muChi2ErrLow",&muChi2ErrLow);
  chain->SetBranchAddress("muChi2ErrHigh",&muChi2ErrHigh);
  chain->SetBranchAddress("muAICErrLow",&muAICErrLow);
  chain->SetBranchAddress("muAICErrHigh",&muAICErrHigh);
  ...
  for (int e=0; e<chain->GetEntries(); e++){
    ...
    for (unsigned int el=0; el<truthModel->size(); el++){
      ...
      fillHist(truth,"PaulErr",muPaulErrLow->at(el),muPaulErrMap,resBins,resL,resH);
      fillHist(truth,"PaulErr",muPaulErrHigh->at(el),muPaulErrMap,resBins,resL,resH);
      fillHist(truth,"Chi2Err",muChi2ErrLow->at(el),muChi2ErrMap,resBins,resL,resH);
      fillHist(truth,"Chi2Err",muChi2ErrHigh->at(el),muChi2ErrMap,resBins,resL,resH);
      fillHist(truth,"AICErr",muAICErrLow->at(el),muAICErrMap,resBins,resL,resH);
      fillHist(truth,"AICErr",muAICErrHigh->at(el),muAICErrMap,resBins,resL,resH);

      // fillPullHist(truth,"FabPull",expmean,muFab->at(el),muFabErr->at(el),muFabPullMap);
      fillPullHist(truth,"PaulPull",expmean,muPaul->at(el),muPaulErrLow->at(el),muPaulPullMap);
      fillPullHist(truth,"PaulPull",expmean,muPaul->at(el),muPaulErrHigh->at(el),muPaulPullMap);
      fillPullHist(truth,"Chi2Pull",expmean,muChi2->at(el),muChi2ErrLow->at(el),muChi2PullMap);
      fillPullHist(truth,"Chi2Pull",expmean,muChi2->at(el),muChi2ErrHigh->at(el),muChi2PullMap);
      fillPullHist(truth,"AICPull",expmean,muAIC->at(el),muAICErrLow->at(el),muAICPullMap);
      fillPullHist(truth,"AICPull",expmean,muAIC->at(el),muAICErrHigh->at(el),muAICPullMap);
