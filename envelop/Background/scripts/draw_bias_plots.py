...
parser.add_option("-c","--cat",type="string")
...
if options.remakeHists:
  ...
  r.gROOT.ProcessLine('.L ~/CMSSW_7_4_7/src/flashggFinalFit/Background/scripts/make_bias_hists.C+g')
...
def drawHists():
  ...
  scanTypes=['Paul','Chi2','AIC']
  graphCol=[r.kAzure+5,r.kPink-4,r.kTeal+9]
  label=['Envelope (no pen)','Envelope (1/dof pen)','Envelope (2/dof pen)']
  ...
  for c, stype in enumerate(scanTypes):
    ...
    for ptype in plotTypes:
      ...
      graphDict[stype][ptype].SetTitle('Category %s - %s'%(options.cat,ptype))
  ...
  for type in ['','Err','Pull']:
    for i, truth in enumerate(truth_mods):
      ...
      for c, stype in enumerate(scanTypes):
        print '%s_mu%s%s'%(truth,stype,type)
        print truth 
        print stype
        print type
        hist = file.Get('%s_truth_mu%s%s'%(truth,stype,type))        
        hist.GetMean()
      ...
      for c, stype in enumerate(scanTypes):
        ...
        hist = file.Get('%s_truth_mu%s%s'%(truth,stype,type))
        hist.SetTitle('Category %s - Truth %s'%(options.cat,name))
      ...
      if type=='':
        ...
        #if not options.skipANlines: muBiasFabNote.SetPoint(i,0.5+i,fabDict['truth_%s_cat%s'%(name,options.cat)])
  ...
  if options.is2011:
    muHistDummy.SetTitle("2011 Category %s - #mu_{gen}=%3.1f"%(options.cat,options.expectSignal))
  else:
    muHistDummy.SetTitle("Category %s - #mu_{gen}=%3.1f"%(options.cat,options.expectSignal))
