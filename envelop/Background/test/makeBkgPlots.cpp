...
int main(int argc, char* argv[]){
  ...
  desc.add_options()
    ...
    ("sigFileName,s", po::value<string>(&sigFileName), 																		"Input sig file name")
  ...
  //RooAbsData *data = (RooDataSet*)inWS->data("CMS_hzg_mass");
  ...
  RooCategory *mcat = 0;
  if (isMultiPdf) {
    mpdf = (RooMultiPdf*)inWS->pdf(Form("CMS_hzg%s_%dTeV_bkgshape",catname.c_str(),sqrts));

    if (!mpdf){cout << "[ERROR] "<< "Can't find multipdfs (" << Form("CMS_hzg_%s_%dTeV_bkgshape",catname.c_str(),sqrts) << ") "<<endl; exit(0);}
    mcat = (RooCategory*)inWS->cat(Form("pdfindex%s_%dTeV",catname.c_str(),sqrts));
    if(!mcat){cout<< "multicat ("<<Form("pdfindex%s_%dTeV",catname.c_str(),sqrts) <<")" << endl;exit(0);}
  }
  else {
  ...
  if (isMultiPdf || verbose_) plotAllPdfs(mzg,data,mpdf,mcat,Form("%s/allPdfs_%s",outDir.c_str(),catname.c_str()),cat,unblind, isFlashgg_, flashggCats_);
  ...
  */
  ...
  if (doSignal){
    ...
    if (SignalType==1) {
      ...
      TH1F *sigHist = (TH1F*)gghHist->Clone(Form("signal_m125_%s",catname.c_str()));
      sigHist->Add(gghHist);
      sigHist->Add(vbfHist);
  ...
  latex->DrawLatex(0.15,0.85,Form("%s_%s",type.c_str(),flashggCats_[cat].c_str()));
  ...
  pad2->cd();
  TH1 *hdummy = new TH1D("hdummyweight","",55,100,170);
  ...
  TLine *line3 = new TLine(100,0.,170,0.);
