...
int main(int argc, char *argv[]) {
  ...
  TString dirname = prefix+"_temp";
  gSystem->mkdir(dirname,true);
  gSystem->cd(dirname);
  gSystem->pwd();
  const double leftpos = 102.0;
   
  cout<<filename_[0]<<endl;
  ...
  RooRealVar *mass = win->var("CMS_hzg_mass");
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    RooAbsReal *catnorm = win->var(TString::Format("shapeBkg_bgr_%s__norm",chan->getLabel()));
  ...
  chain->Add("~/CMSSW_7_4_7/src/flashggFinalFit/Plots/FinalResults/test/higgsCombinecombout_step2_done*.root"); //FIXME make this dynamic ?
  ...
  for (int i=0; i<(chan->numTypes()+2); ++i) {
    ...
    if (i<ncats) {
     ...
     // RooDataSet *catdata = (RooDataSet*)data->reduce(TString::Format("CMS_channel=='%s'",chan->getLabel()));
      ...
      catdatam = data;
      printf("[INFO] catdata -- Unweighted data -- %d --- entries = %5f\n",i,catdatam->sumEntries());
    }
    else if (i==(ncats+1)) {
      catdatam = wdata;
      printf("[INFO] catdata -- Weighted data -- %d--- entries = %5f\n",i,catdatam->sumEntries());
    }
...
//     pad2->SetBorderMode(0);
    ...
    //RooPlot *catplot = mass->frame(115.,180.,80);
