...
int main(int argc, char* argv[]){
  ...
  desc.add_options()
    ...
    ("sigwsname", po::value<string>(&sigWSName)->default_value(""),            "Signal workspace name")
    ("bkgwsname", po::value<string>(&bkgWSName)->default_value("multipdf"),            "Background workspace name")
    ...
    ("ptype,p", po::value<string>(&ptype),                                                             "electron or muon")
...
std::cout << "DEBUG LC 0 " <<std::endl;
  ...
  std::istringstream sigfiles(sigFileName);
  std::string temp;
  cout<<sigFileName<<endl;
  std::vector <string> SigFiles;SigFiles.clear();
  while(std::getline(sigfiles,temp,','))
    {
      SigFiles.push_back(temp);
    }
  TFile *sigFile_ggF = TFile::Open(SigFiles[0].c_str());
  TFile *sigFile_VBF = TFile::Open(SigFiles[1].c_str());
  TFile *sigFile_ttH = TFile::Open(SigFiles[2].c_str());
  TFile *sigFile_ZH = TFile::Open(SigFiles[3].c_str());
  TFile *sigFile_WH = TFile::Open(SigFiles[4].c_str());
  //RooWorkspace *bkgWS = (RooWorkspace*)bkgFile->Get("cms_hzg_workspace");
  RooWorkspace *bkgWS = (RooWorkspace*)bkgFile->Get(bkgWSName.c_str());
  RooWorkspace *sigWS_ggF = (RooWorkspace*)sigFile_ggF->Get(sigWSName.c_str());
  RooWorkspace *sigWS_VBF = (RooWorkspace*)sigFile_VBF->Get(sigWSName.c_str());
  RooWorkspace *sigWS_ttH = (RooWorkspace*)sigFile_ttH->Get(sigWSName.c_str());
  RooWorkspace *sigWS_ZH = (RooWorkspace*)sigFile_ZH->Get(sigWSName.c_str());
  RooWorkspace *sigWS_WH = (RooWorkspace*)sigFile_WH->Get(sigWSName.c_str());


std::cout << "DEBUG LC 1 " <<std::endl;
  if (!bkgWS || !sigWS_ggF||!sigWS_VBF||!sigWS_ttH||!sigWS_WH||!sigWS_ZH){
  ...
  RooRealVar *mass = (RooRealVar*)bkgWS->var("CMS_hzg_mass");
...
std::cout << "DEBUG LC 2.1 " <<std::endl;
  ...
  RooDataSet *data = (RooDataSet*)bkgWS->data(Form("data_obs_%s_cat%d",ptype.c_str(),cat));
  std::cout << "debug lc 2.2.1 , cat "<< cat <<std::endl;
  //RooDataSet *data = (RooDataSet*)bkgWS->data(Form("data_%s_7TeV",cat));
  RooDataHist *dataBinned = (RooDataHist*)bkgWS->data(Form("roohist_data_mass_%s_cat%d",ptype.c_str(),cat));
  //RooDataHist *dataBinned = data;
  //RooDataHist *dataBinned = new RooDataHist(Form("roohist_data_mass_%s",cat.c_str()),Form("roohist_data_mass_%s",cat.c_str()),RooArgSet(*mass));
  std::cout << "debug lc 2.2.1.1 " <<std::endl;
  RooDataSet *sigMC = (RooDataSet*)sigWS_ggF->data(Form("%s_ggF_%d_cat%d",ptype.c_str(),expectSignalMass,cat));
  std::cout << "debug lc 2.2.2 " <<std::endl;
  RooDataSet *sigMC_vbf = (RooDataSet*)sigWS_VBF->data(Form("%s_VBF_%d_cat%d",ptype.c_str(),expectSignalMass,cat));
  RooDataSet *sigMC_wh = (RooDataSet*)sigWS_WH->data(Form("%s_WH_%d_cat%d",ptype.c_str(),expectSignalMass,cat));
  RooDataSet *sigMC_zh = (RooDataSet*)sigWS_ZH->data(Form("%s_ZH_%d_cat%d",ptype.c_str(),expectSignalMass,cat));
  std::cout << "debug lc 2.2.3 " <<std::endl;
  RooDataSet *sigMC_tth = (RooDataSet*)sigWS_ttH->data(Form("%s_ttH_%d_cat%d",ptype.c_str(),expectSignalMass,cat));
  ...
  //RooExtendPdf *ggh_pdf = (RooExtendPdf*)sigWS->pdf(Form("sigpdfsmrel_%s_7TeV_ggh",cat));
  //RooExtendPdf *vbf_pdf = (RooExtendPdf*)sigWS->pdf(Form("sigpdfsmrel_%s_7TeV_vbf",cat));
  //RooExtendPdf *wzh_pdf = (RooExtendPdf*)sigWS->pdf(Form("sigpdfsmrel_%s_7TeV_wzh",cat));
  //RooExtendPdf *tth_pdf = (RooExtendPdf*)sigWS->pdf(Form("sigpdfsmrel_%s_7TeV_tth",cat));
  //RooAbsPdf *sigPdf = new RooAddPdf(Form("sigpdfsmrel_%s_7TeV",cat),Form("sigpdfsmrel_%s_7TeV",cat),RooArgList(*ggh_pdf,*vbf_pdf,*wzh_pdf,*tth_pdf));
 ...
 if (!dataBinned || !sigMC){
  ...
  for (vector<pair<int,pair<string,string> > >::iterator it=toysMap.begin(); it!=toysMap.end(); it++){
    ...
    if (it->first==-2) { // this is a keys pdf toy
      ...
      toysModel.addBkgPdf("KeysPdf",0,Form("truth_%s_%s_%d",it->second.second.c_str(),ptype.c_str(),cat),false);
      continue;
    }
    ...
    toysModel.addBkgPdf(it->second.second,it->first,Form("truth_%s_%s_%d",it->second.first.c_str(),ptype.c_str(),cat),false); 
  }
  //toysModel.printPdfs();
  ...
  for (vector<pair<int,pair<string,string> > >::iterator it=fabianMap.begin(); it!=fabianMap.end(); it++){
    fabianModel.addBkgPdf(it->second.second,it->first,Form("fabian_%s_%s_%d",it->second.first.c_str(),ptype.c_str(),cat),false); 
  }
...
std::cout << "DEBUG LC 4 " <<std::endl;
  ...
  for (vector<pair<int,pair<string,string> > >::iterator it=paulMap.begin(); it!=paulMap.end(); it++){
    ...
    paulModel.addBkgPdf(it->second.second,it->first,Form("paul_%s_%s_%d",it->second.first.c_str(),ptype.c_str(),cat),false); 
 }
  paulModel.setSignalPdfFromMC(sigMC);
  //paulModel.setSignalPdf(sigPdf);
  paulModel.makeSBPdfs(true);
  ...
  if (!skipPlots) toysModel.plotPdfsToData(dataBinned,55,Form("%s/plots/truthToData/datafit_mu%3.1f",outDir.c_str(),expectSignal),false);
  toysModel.setSignalModifierVal(expectSignal);
  toysModel.setSignalModifierConstant(false);
  ...
  for (int toy=0; toy<ntoys; toy++){
    ...
    if (throwHybridToys) {
      ...
      if (!skipPlots) toysModel.plotToysWithPdfs(Form("%s/plots/toys/job%d_toy%d",outDir.c_str(),jobn,toy),55,false);
      if (!skipPlots) toysModel.plotHybridToy(Form("%s/plots/toys/job%d_toy%d",outDir.c_str(),jobn,toy),55,switchMass,switchFunc,false);
    }
    else {
    ...
    for (map<string,RooAbsData*>::iterator it=toys.begin(); it!=toys.end(); it++){
      ...
      toysModel.plotPdfsToData(it->second,55,Form("%s/plots/toys/job%d_toy%d",outDir.c_str(),jobn,toy),true,"NONE");
      //if (!skipPlots) fabianProfiler.plotNominalFits(it->second,mass,55,Form("%s/plots/toys/job%d_toy%d_fit_fab",outDir.c_str(),jobn,toy));
      if (!skipPlots) paulProfiler.plotNominalFits(it->second,mass,55,Form("%s/plots/toys/job%d_toy%d_fit_paul",outDir.c_str(),jobn,toy));
      //continue;
      // --------------------------------
      ...
      pair<double,map<string,TGraph*> > fabianEnvelope = fabianProfiler.computeEnvelope(fabianMinNlls,Form("fabEnvelope_job%d_%s_%s_%d_toy%d",jobn,it->first.c_str(),ptype.c_str(),cat,toy),0.);
      if (!skipPlots) fabianProfiler.plot(fabianEnvelope.second,Form("%s/plots/envelopeNlls/nlls_fab_%s_%s_%d_toy%d",outDir.c_str(),it->first.c_str(),ptype.c_str(),cat,toy));
      */
      // get Paul envelopes

      pair<double,map<string,TGraph*> > paulMinNlls = paulProfiler.profileLikelihood(it->second,mass,mu,mu_low,mu_high,mu_step);

      pair<double,map<string,TGraph*> > paulEnvelope = paulProfiler.computeEnvelope(paulMinNlls,Form("paulEnvelope_job%d_%s_%s_%d_toy%d",jobn,it->first.c_str(),ptype.c_str(),cat,toy),0.);
      if (!skipPlots) paulProfiler.plot(paulEnvelope.second,Form("%s/plots/envelopeNlls/nlls_paul_%s_%s_%d_toy%d",outDir.c_str(),it->first.c_str(),ptype.c_str(),cat,toy));
      pair<double,map<string,TGraph*> > chi2Envelope = paulProfiler.computeEnvelope(paulMinNlls,Form("chi2Envelope_job%d_%s_%s_%d_toy%d",jobn,it->first.c_str(),ptype.c_str(),cat,toy),1.);
      if (!skipPlots) paulProfiler.plot(chi2Envelope.second,Form("%s/plots/envelopeNlls/nlls_chi2_%s_%s_%d_toy%d",outDir.c_str(),it->first.c_str(),ptype.c_str(),cat,toy));
      pair<double,map<string,TGraph*> > aicEnvelope = paulProfiler.computeEnvelope(paulMinNlls,Form("aicEnvelope_job%d_%s_%s_%d_toy%d",jobn,it->first.c_str(),ptype.c_str(),cat,toy),2.);
      if (!skipPlots) paulProfiler.plot(aicEnvelope.second,Form("%s/plots/envelopeNlls/nlls_aic_%s_%s_%d_toy%d",outDir.c_str(),it->first.c_str(),ptype.c_str(),cat,toy));
