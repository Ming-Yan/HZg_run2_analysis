...
int mzg_low =115;//mod-zg
int mzg_high =170;//mod-zg
int nBinsForMass = 4*(mzg_high-mzg_low);
...
double getProbabilityFtest(double chi2, int ndof,RooAbsPdf *pdfNull, RooAbsPdf *pdfTest, RooRealVar *mass, RooDataSet *data, std::string name){
  ...
  TLegend *leg = new TLegend(0.2,0.6,0.4,0.87); leg->SetFillColor(-1);
  leg->SetTextFont(42);
...
void plot(RooRealVar *mass, RooAbsPdf *pdf, RooDataSet *data, string name,vector<string> flashggCats_, int status, double *prob){
  ...
  *prob = getGoodnessOfFit(mass,pdf,data,name);
  ...
  mass->setRange("unblindReg_1",mzg_low,120);
  mass->setRange("unblindReg_2",130,mzg_high);
  if (BLIND) {
    data->plotOn(plot,Binning(mzg_high-mzg_low),CutRange("unblindReg_1"));
    data->plotOn(plot,Binning(mzg_high-mzg_low),CutRange("unblindReg_2"));
    data->plotOn(plot,Binning(mzg_high-mzg_low),Invisible());
  }
  else data->plotOn(plot,Binning(mzg_high-mzg_low));
...
void plot(RooRealVar *mass, RooMultiPdf *pdfs, RooCategory *catIndex, RooDataSet *data, string name, string type,vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  
  int color[7] = {kBlue,kRed,kMagenta,kGreen+1,kOrange+7,kAzure+10,kBlack};
  TLegend *leg = new TLegend(0.54,0.55,0.95,0.9);
  leg->SetFillColor(-1);
  leg->SetLineColor(1);
  RooPlot *plot = mass->frame();

  mass->setRange("unblindReg_1",mzg_low,120);//mod-zg
  mass->setRange("unblindReg_2",130,mzg_high);//mod-zg
  if (BLIND) {
    data->plotOn(plot,Binning(mzg_high-mzg_low),CutRange("unblindReg_1"));
    data->plotOn(plot,Binning(mzg_high-mzg_low),CutRange("unblindReg_2"));
    data->plotOn(plot,Binning(mzg_high-mzg_low),Invisible());
  ...
  plot->SetTitle(Form("%s Category %s",type.c_str(),flashggCats_[cat].c_str()));
  if (BLIND) plot->SetMinimum(0.0001);
  ...
  CMS_lumi( pad1, 0, 0);
  ///start extra bit for ratio plot///
  TH1D *hbplottmp = (TH1D*) pdf->createHistogram("hbplottmp",*mass,Binning(mzg_high-mzg_low,mzg_low,mzg_high));
  ...
  pad2->cd();
  TH1 *hdummy = new TH1D("hdummyweight","",mzg_high-mzg_low,mzg_low,mzg_high);
  ...
  TLine *line3 = new TLine(mzg_low,0.,mzg_high,0.);
...
void plot(RooRealVar *mass, map<string,RooAbsPdf*> pdfs, RooDataSet *data, string name, string type, vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  ...
  if(flashggCats_.size() >0){
    leg->AddEntry(datLeg,Form("Data - %s %s",type.c_str(),flashggCats_[cat].c_str()),"LEP");
  } else {
    leg->AddEntry(datLeg,Form("Data - %s %s",type.c_str(),flashggCats_[cat].c_str()),"LEP");
  }
  int i=0;
  ...
  plot->SetTitle(Form(" %s %s",type.c_str(),flashggCats_[cat].c_str()));
  if (BLIND) plot->SetMinimum(0.0001);
  plot->Draw();
  leg->Draw("same");
  CMS_lumi( canv, 0, 0);
...
int main(int argc, char* argv[]){
  ...
  desc.add_options()
    ...
    ("type,t", po::value<string>(&type),														"Type of electron or muon")
    ("saveMultiPdf", po::value<string>(&outfilename),         					"Save a MultiPdf model with the appropriate pdfs")
    ("runFtestCheckWithToys", 									"When running the F-test, use toys to calculate pvals (and make plot) ")
    ("period,p", po::value<int>(&period),         "Periods ")
    ...
    ("flashggCats,f", po::value<string>(&flashggCatsStr_)->default_value("cat0,cat1,cat2,cat3,cat4,cat501,cat502,cat6789"),       "Flashgg category names to consider")
  ...
  if (isFlashgg_==1)	ncats= flashggCats_.size();
  ...
  RooWorkspace *inWS;
  if(isFlashgg_){
    if (isData_)inWS = (RooWorkspace*)inFile->Get("w");//mod-zg
    else inWS = (RooWorkspace*)inFile->Get("cms_hgg_workspace");
  } else inWS = (RooWorkspace*)inFile->Get("cms_hgg_workspace");//FIXME
  ...
  if (saveMultiPdf){
    ...
    if (isFlashgg_){
      intL = new RooRealVar("intL","intL",35.9);
      sqrts = new RooRealVar("SqrtS","SqrtS",13); 
  ...
  FILE *resFile ;
  resFile = fopen(Form("%s/fTestResults_%s_%s.txt",outDir.c_str(),type.c_str(),flashggCats_[0].c_str()),"w");
  ...
  RooRealVar *mass = (RooRealVar*)inWS->var("CMS_hzg_mass");//mod-zg
  ...
  for (int cat=startingCategory; cat<ncats; cat++){
    ...
    string catname;
    if (isFlashgg_)catname = Form("%s_%s_%d",type.c_str(),flashggCats_[cat].c_str(),period);
	else  catname = Form("cat%d",cat);
    RooDataSet *dataFull;
    ...
    mass->setBins(nBinsForMass);
    RooDataSet *data;
    //	RooDataHist thisdataBinned(Form("roohist_data_mass_cat%d",cat),"data",*mass,RataFull);
    //	RooDataSet *data = (RooDataSet*)&thisdataBinned;
    string thisdataBinned_name;

    if ( isFlashgg_){
      thisdataBinned_name =Form("roohist_data_mass_%s_%s_%d",type.c_str(),flashggCats_[cat].c_str(),period);
      //data = (RooDataSet*)&thisdataBinned;
      //		std::cout << "debug " << thisdataBinned.GetName() << std::endl;
      
      data = (RooDataSet*)dataFull;
    } else {
    ...
    fprintf(resFile,"\\multicolumn{4}{|c|}{\\textbf{Category %s}} \\\\\n",flashggCats_[cat].c_str());
    fprintf(resFile,"\\hline\n");
	...
	RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("ftest_pdf_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()));
	if (!bkgPdf){
	  ...
	  if (prev_pdf!=NULL){
	    prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,data
				       ,Form("%s/Ftest_from_%s_%d_%s_%s_%d.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period));
	  ...
	  if (!saveMultiPdf) plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
	...
	while (prob<upperEnvThreshold){
	  RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()));
	  if (!bkgPdf ){
	    // assume this order is not allowed
	    ...
	    double gofProb =0; 
	    plot(mass,bkgPdf,data,Form("%s/%s%d_%s_%s_%d.pdf",outDir.c_str(),funcType->c_str(),order,type.c_str(),flashggCats_[cat].c_str(),period),flashggCats_,fitStatus,&gofProb);
    ...
    plot(mass,pdfs,data,Form("%s/truths_%s_%s_%d",outDir.c_str(),type.c_str(),flashggCats_[cat].c_str(),period),type.c_str(),flashggCats_,cat);

    if (saveMultiPdf){
      // Put selectedModels into a MultiPdf
      string catindexname;
      if (isFlashgg_){
	catindexname = Form("pdfindex_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(), period,ext.c_str());
      } else {
	catindexname = Form("pdfindex_%s_%s_%d_%s",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str());
      }
      RooCategory catIndex(catindexname.c_str(),"c");
      RooMultiPdf *pdf = new RooMultiPdf(Form("CMS_hzg_%s_%s_%d_%s_bkgshape",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()),"all pdfs",catIndex,storedPdfs);
      RooRealVar nBackground(Form("CMS_hzg_%s_%s_%d_%s_bkgshape_norm",type.c_str(),flashggCats_[cat].c_str(),period,ext.c_str()),"nbkg",data->sumEntries(),0,10E8);
      ...
      outputws->import(catIndex);
      //std::cout<<outputws->Print()<<std::endl;
      outputws->import(dataBinned);
      outputws->import(*dataFull);
      plot(mass,pdf,&catIndex,data,Form("%s/multipdf_%s",outDir.c_str(),catname.c_str()),type,flashggCats_,cat,bestFitPdfIndex);
      outputws->Print();
    }
