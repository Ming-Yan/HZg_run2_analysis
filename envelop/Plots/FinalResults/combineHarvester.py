...
def strtodict(lstr):
  print "[INFO] string to dictionary"
  retdict = {}
...
def getSortedCats():
    ...
    for m in range(120,131,1):
        tempcard = opts.datacard+"_"+str(m)+ ".txt"
        cats = set()
        f = open(tempcard)
...
def splitCard():
  ...
  for m in range(120,131,1):
      opts.datacard = "card_VBFMVA_v2_all"
      tmpcard = opts.datacard+"_"+str(m)+".txt"
      f = open(tmpcard)
      allCats = set()
      ...
      veto=veto[1:]
      splitCardName = tmpcard.replace('.txt','')
      for cat in opts.splitChannels: splitCardName += '_'+cat
      splitCardName += '.txt'
      print 'combineCards.py --xc="%s" %s > %s'%(veto,tmpcard,splitCardName)
      system('combineCards.py --xc="%s" %s > %s'%(veto,tmpcard,splitCardName))
      opts.datacard = splitCardName
      removeRelevantDiscreteNuisances()
def makeFloatMHCard():
	for m in range(120,131,1):            
            tmpcard=opts.datacard + "_" + str(m) + "mhRange.txt";
            f1 = open("%s_%d.txt"%(opts.datacard,m), "r")
            f2 = open(tmpcard, "w")
            for line in f1:
...
def makeTheoOnlyCard():
	...
	for m in range(120,131,1):
	  ...
	  tmp = opts.datacard + "_" + str(m) + ".txt";
	  newcardname = tmp.replace('.txt','_theoonly.txt') 
...
def makeJustThisSystCard():
  ...
  for m in range(120,131,1):
	  assert(opts.datacard.endswith('.txt'))
	  tmp = opts.datacard + "_" + str(m) + ".txt";
	  newcardname = tmp.replace('.txt','_%soonly.txt'%opts.justThisSyst[0]) 
	  outf = open(newcardname,'w')
...
#		assert(opts.datacard.endswith('.txt'))
            tmp = opts.datacard + "_" + str(m) + ".txt"
            newcardname = opts.datacard + "_" + str(m) +"_statonly.txt"
            outf = open(newcardname,'w')
            inf = open(tmp, 'r')
            for line in inf.readlines():
                line_els = line.split()
...
#		assert(opts.datacard.endswith('.txt'))
            tmp = opts.datacard + "_" + str(m) + ".txt";
            newcardname = tmp.replace('.txt','_%s.txt'%opts.label) 
            outf = open(newcardname,'w')
            inf = open(tmp)
            for line in inf.readlines():
                line_els = line.split()
...
#	  assert(opts.datacard.endswith('.txt'))
	  tmp = opts.datacard + "_" + str(m) + ".txt";
	  newcardname = tmp.replace('.txt','_noglob.txt'%opts.label) 
	  outf = open(newcardname,'w')
	  inf = open(opts.datacard)
	  for line in inf.readlines():
		line_els = line.split()
		if line.startswith('kmax'): line = line.replace(line_els[1],'*')
		if line.startswith('CMS_hzg_globalscale'): continue
		else: outf.write(line)
	  inf.close()
	  outf.close()
	  opts.datacard = newcardname 
...
def writePreamble(sub_file):
  ...
  for file in opts.files.split(','):
      if opts.splitChannels is not None:
          print opts.splitChannels
          if opts.splitChannels.contains("ele") and file.contains("ele"):
              sub_file.write('cp -p %s .\n'%os.path.abspath(file))
          elif opts.splitChannels.contains("mu") and file.contains("mu"):
              sub_file.write('cp -p %s .\n'%os.path.abspath(file))
  tmpcard =  opts.datacard+"_1*.txt"
  sub_file.write('cp -p %s . \n'%os.path.abspath(tmpcard))
  tmpcard =  opts.datacard+"_1*.root"
  sub_file.write('cp -p %s . \n'%os.path.abspath(tmpcard))
...
def writeAsymptotic():
  ...
  for j, mass_set in enumerate(opts.masses_per_job):
    ...
    for mass in mass_set:
      exec_line +=  'combine %s_%d.txt -M Asymptotic -m %6.2f --cminDefaultMinimizerType=Minuit2'%(opts.datacard,int(mass),mass)
...
def writeAsymptoticGrid():
  ...
  if not opts.skipWorkspace:
	...
	for m in range(120,131,1):
		tempcard = opts.datacard+"_"+str(m)+"_"+opts.label+".txt"
		ws_exec_line = 'text2workspace.py %s -o %s.root'%(os.path.abspath(tempcard),os.path.abspath(tempcard).replace('.txt','.root')) 
		#print ws_exec_line
		system(ws_exec_line)
  ...
  for j, mass_set in enumerate(opts.masses_per_job):
    for mass in mass_set:
      system('python $CMSSW_BASE/src/HiggsAnalysis/CombinedLimit/test/makeAsymptoticGrid.py -w %s_%d.root -m %6.2f -n 10 -r %3.1f %3.1f --runLimit --nCPU=3 -d %s'%(opts.datacard,int(mass),mass,opts.muLow,opts.muHigh,os.path.abspath(opts.outDir)))
  ...
  for j, mass_set in enumerate(opts.masses_per_job):
      ...
      if opts.toysFile:  
          ...
          for mass in mass_set:
              ...
              writePreamble(file)
              exec_line = ''
              m = opts.datacard[19:21]
        #if m is not mass:
        #   opts.datacard.replace(m,
...
def writeChannelCompatibility():
  ...
  writePreamble(file)
  exec_line = 'combine %s_%d.txt -M ChannelCompatibilityCheck -m %6.2f --rMin=-25. --saveFitResult --cminDefaultMinimizerType=Minuit2'%(opts.datacard,opts.mh)
  writePostamble(file,exec_line)
...
def writeSingleGenerateOnly():
  ...
  writePreamble(file)
  for m in range(120,131,1):
    exec_line = 'combine %s_%d.txt -M GenerateOnly -m %6.2f --saveToys '%(opts.datacard,m,opts.mh)
    if opts.expected: exec_line += ' -t -1'
    if opts.expectSignal: exec_line += ' --expectSignal=%3.1f'%opts.expectSignal
    if opts.expectSignalMass: exec_line += ' --expectSignalMass=%6.2f'%opts.expectSignalMass
  writePostamble(file,exec_line)
...
def writeMultiDimFit(method=None,wsOnly=False):
    ...
    while mypath != "":
        ...
        if opts.method=="PerTagChannelCompatibility" and catsMap=="":
            ...
            catsMap =" --PO verbose "
            for cat in cats:
                catsMap = catsMap + " --PO 'map=%s/.*hzg_13TeV:r_%s[1,-5,5]'"%(cat,cat)
                perTagChCompPOIs.append("r_%s"%cat)
                print '[INFO] Writing MultiDim Scan %s' %catsMap
    ...
    "RDiffXsScan"  : "--floatOtherPOIs=1 -P %s"%(opts.poix), # need to add option to run specific topologic categories
        ...
        if (opts.label):
         makeLabelledCard()
        if not opts.skipWorkspace:
          for m in range(125,126,1):
            datacardname = os.path.basename(opts.datacard).replace('.txt','')
            print 'Creating workspace for %s...'%method
            if opts.label is not None:
                tmpcard = opts.datacard + "_" + str(m) + "_" + opts.label
            else:
                tmpcard = opts.datacard + "_" + str(m)
            print tmpcard   
            exec_line = 'text2workspace.py %s.txt -o %s.root %s'%(os.path.abspath(tmpcard),os.path.abspath(tmpcard),ws_args[method]) 
			
            if opts.postFit:
                tmpcard = opts.datacard + "_" + str(m)
                exec_line += ' && combine -m %.2f -M MultiDimFit --saveWorkspace -n %s_postFit %s%s.root' % ( opts.mh, datacardname+method, os.path.abspath(tmpcard),method )
                exec_line += ' && cp higgsCombine%s_postFit.MultiDimFit.mH%.2f.root %s%s_postFit.root' % ( tmpcard+method, opts.mh, os.path.abspath(tmpcard), method)
            ...
            if opts.freezeAll:
                if opts.label is not None:
                    cards = opts.datacard + "_" + str(m) + "_" + opts.label
                else:
                    cards = opts.datacard + "_" + str(m)
            ...
            if opts.postFit:
                tmpcard = tmpcard.replace('.txt',method+'_postFit.root')
        ...
        for i in range(opts.jobs):
            for m in range(125,126,1):
                ...
                if opts.label is not None:
                    ...
                    tmpcard = opts.datacard + "_" + str(m)
                if opts.postFit:
                    tmpcard = tmpcard+"_postFit"
                exec_line = 'combine %s.root  -M MultiDimFit --cminDefaultMinimizerType Minuit2 --cminDefaultMinimizerAlgo migrad --algo=grid  %s --points=%d --firstPoint=%d --lastPoint=%d -n %sJob%d '%(tmpcard,combine_args[method],opts.pointsperjob*opts.jobs,i*opts.pointsperjob,(i+1)*opts.pointsperjob-1,method,i)
                ...
                if getattr(opts,"mh",None): exec_line += ' -m %6.2f'%m
                if opts.expected: exec_line += ' -t -1 '
                print exec_line
	       #exec_line += ' --verbose -1 ' # make very quiet
		exec_line += ' --verbose -1 --saveSpecifiedIndex=pdfindex_ele_cat1_13TeV,pdfindex_ele_cat2_13TeV,pdfindex_ele_cat3_13TeV,pdfindex_ele_cat4_13TeV,pdfindex_ele_cat501_13TeV,pdfindex_ele_cat502_13TeV,pdfindex_mu_cat1_13TeV,pdfindex_mu_cat2_13TeV,pdfindex_mu_cat3_13TeV,pdfindex_mu_cat4_13TeV,pdfindex_mu_cat501_13TeV,pdfindex_mu_cat502_13TeV,pdfindex_ele_mu_cat6789_13TeV'
...
if opts.hadd:
  ...
  for line in datfile.readlines():
    ...
    if line.startswith('datacard'): 
      ...
      defaults.datacard = 'card_' + opts.datacard 
