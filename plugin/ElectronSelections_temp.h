  ...
  for (Int_t i=0; i<nEle; ++i)
    {
      ...
      if (fabs(eleEta[i]) > 2.5) continue;//remove
      if (fabs(eleSCEta[i]) > 2.5) continue;
      if (fabs(eleSCEta[i]) > 1.4442 && fabs(eleSCEta[i]) < 1.566) continue;

      if ((eleIDbit[i] >> 5  & 1) == 1) continue;
      accepted.push_back(i);
    }
}

//electron MVA selection
void ElectronID16NonTrgMVA(TreeReader &data, Int_t WP, vector<int> &Zee, vector<TLorentzVector> &multz)
{
...
void ElectronID17NonTrgMVA_mod(TreeReader &data, Int_t WP, vector<int> &accepted)
{
  ...
  float* eleSIP = data.GetPtrFloat("eleSIP");
  //2017 MVA cut -w/ ISO
  Float_t cut_mva_HZZ_ISO[3]  = {-0.0339139,-0.0184520,-0.385655};
  Float_t lowcut_mva_HZZ_ISO[3]  = {0.573952,0.550463,0.592463};
...
void ElectronID17NonTrgMVA(TreeReader &data, Int_t WP, vector<int> &Zee)
{
...
void ElectronHLTSafeAndTight2016(TreeReader &data, vector<int> &accepted)
{
  ...
  // fix me with correct rho fixedGridRhoFastjetCentralCalo in next tag
  float  rho                    = data.GetFloat("rho");
  Int_t  nEle                   = data.GetInt("nEle");
  float* elePt                   = data.GetPtrFloat("elePt");
  //float* elePt                   = data.GetPtrFloat("eleCalibPt");
  ...
  //float* eleGSFChi2              = data.GetPtrFloat("eleGSFChi2");  // fix me in next tag !
  UShort_t* eleIDbit             = (UShort_t*) data.GetPtrShort("eleIDbit");

  // https://twiki.cern.ch/twiki/bin/view/CMS/CutBasedElectronIdentificationRun2#HLT_safe_selection_for_2016_data
  Float_t cut_sieie[2]    = {0.011, 0.031};
  Float_t cut_dEtaSeed[2] = {0.004, 1e30};
  Float_t cut_dPhi[2]     = {0.02,  1e30};
  Float_t cut_hoe[2]      = {0.06,  0.065};
  Float_t cut_OEmOP[2]    = {0.013, 0.013};
  Float_t cut_ecalIso[2]  = {0.16,  0.12};
  Float_t cut_hcalIso[2]  = {0.12,  0.12};
  Float_t cut_trkIso[2]   = {0.08,  0.08};
  Float_t cut_chi2[2]     = {1e30,  3.0};

  Float_t EA_ecal[2]      = {0.165, 0.132};
  Float_t EA_hcal[2]      = {0.060, 0.131};

  for (Int_t i=0; i<nEle; ++i)
    {

      if (elePt[i] < 30) continue;
      if (fabs(eleEta[i]) > 2.1) continue;
      if (fabs(eleSCEta[i]) > 1.4442 && fabs(eleSCEta[i]) < 1.566) continue;
      if ((eleIDbit[i] >> 3 & 1) == 0) continue;

      Int_t iEB = (fabs(eleSCEta[i]) < 1.479) ? 0 : 1;

      // pre-selection
      if (eleSigmaIEtaIEtaFull5x5[i]                      > cut_sieie[iEB])    continue;
      if (fabs(eledEtaseedAtVtx[i])                       > cut_dEtaSeed[iEB]) continue;
      if (fabs(eledPhiAtVtx[i])                           > cut_dPhi[iEB])     continue;
      if (eleHoverE[i]                                    > cut_hoe[iEB])      continue;
      if ((elePFClusEcalIso[i]-rho*EA_ecal[iEB])/elePt[i] > cut_ecalIso[iEB])  continue;
      if ((elePFClusHcalIso[i]-rho*EA_hcal[iEB])/elePt[i] > cut_hcalIso[iEB])  continue;
      if (eleDr03TkSumPt[i]/elePt[i]                      > cut_trkIso[iEB])   continue;
      if (fabs(eleEoverPInv[i])                           > cut_OEmOP[iEB])    continue;
      //if (eleGSFChi2[i]                                   > cut_chi2[iEB])     continue;
