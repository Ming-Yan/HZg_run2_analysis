...
void HZZmuID17forZg(TreeReader &data, vector<int> &accepted, vector<int> &Zmumu, vector<float> &muPt,Int_t channel){
  Zmumu.clear();
  vector<int> tmpmu;
  tmpmu.clear();
  ...
  for (unsigned int j = 0; j < tmptmpmu.size();j++)
    ...
    {
          tmpmu2.SetPtEtaPhiM(muPt[tmptmpmu[k]],muEta[tmptmpmu[k]],muPhi[tmptmpmu[k]],105.7*0.001);
		  // if(tmpmu1.Pt()<20.&&tmpmu2.Pt()<20.) continue;//new criteria	
          tmpmu.push_back(tmptmpmu[k]);
          tmpmu.push_back(tmptmpmu[j]);
    }   
    }
