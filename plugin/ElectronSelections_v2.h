  ...
  for( int ie = 0; ie <nEle ; ie++ )
    {
	  ...
	  if( elePt[ie] < 15 )                        continue;	  if( fabs(eledEtaAtVtx[ie]) >= 999 )         continue;
...
void ElectronID16NonTrgMVA(TreeReader &data, Int_t WP, vector<int> &Zee, vector<TLorentzVector> &multz)
{
 ...
 Zee.clear();
  ...
  //https://twiki.cern.ch/twiki/bin/viewauth/CMS/MultivariateElectronntificationRun2#Non_triggering_electron_MVA_deta
...
void AddEle16Selections(TreeReader &data, Int_t WP, vector<int> &accepted, float lep1Pt, float lep2Pt, Int_t channel)
{
  ...
  float* eleSCEta         = data.GetPtrFloat("eleSCEta");
  float* elePt            = data.GetPtrFloat("eleCalibPt");
  //float* eleIDMVAHZZ      = data.GetPtrFloat("eleIDMVAIso");  //float* eleIDMVA         = data.GetPtrFloat("eleIDMVA");//11_01 ver. origin eleIDMVANonTrg  //float* eleIDMVA         = data.GetPtrFloat("eleIDMVA");//11_01 ver. origin eleIDMVANonTrg
  float* eleIDMVAHZZ      = data.GetPtrFloat("eleIDMVAHZZ");
  float* elePFChIso       = data.GetPtrFloat("elePFChIso");//sumChargedHadronPt
  float* elePFNeuIso      = data.GetPtrFloat("elePFNeuIso");//sumNeutralHadronEt
  float* elePFPhoIso      = data.GetPtrFloat("elePFPhoIso");//sumPhotonEt
...
void ElectronID17NonTrgMVA(TreeReader &data, Int_t WP, vector<int> &Zee)
{
  ...
  //2017 MVA cut -w/ ISO-LooseID
  Float_t cut_mva_HZZ_ISO[3]  = {0.604775,0.628743,0.896462};
  Float_t lowcut_mva_HZZ_ISO[3]  = {-0.145237,-0.0315746,-0.032173 };
  ///HZZZ WP
  /*Float_t cut_mva_HZZ_ISO[3]  = {0.982489,0.969192,0.793498};
  Float_t lowcut_mva_HZZ_ISO[3]  = {0.852169 ,0.826846,0.869376};
  */
...
void ElectronFallIDv2(TreeReader &data, Int_t WP, vector<int> &Zee)
{
...
void ElectronFallIDv2_16(TreeReader &data, Int_t WP, vector<int> &Zee)
{
 ...
 //fallv2 MVA-w/ ISO Loose ID
  Float_t cut_mva[3]  = {-0.145237,-0.0315746,-0.032173 };
  Float_t lowcut_mva[3]  = {0.604775,0.628743,0.896462};
 //fallv2 MVA-w/ ISO Loose ID
  /*Float_t cut_mva[3]  = {0.982489,0.969192,0.793498};
  Float_t lowcut_mva[3]  = {0.852169 ,0.826846,0.869376};
  */
  ...
  for (Int_t i=0; i<nEle; ++i)
    {
      ...
      if (fabs(eleD0[i]) > cut_d0[iEB]) continue;
      if (fabs(eleDz[i]) > cut_dz[iEB]) continue;
      //if (cut==true&&eleSIP[i] >= 4.) continue;
      ...
      //MVA cut
      if (elePt[i] >= 10. && eleIDMVAIso[i] < cut_mva[cat]) continue;
      if (elePt[i] <= 10. &&eleIDMVAIso[i] < lowcut_mva[cat]) continue;
...
void AddEleIDv2(TreeReader &data, Int_t WP, vector<int> &accepted, float lep1Pt, float lep2Pt, Int_t channel)
{
  accepted.clear();
  vector<int> passUnsorted;
  passUnsorted.clear();
