	...
	for(unsigned int j = i+1; j < jetID.size(); j++)
	  {
		...
		if(fabs(Zeppen) > 2.5) continue;
	      ...
	      {
			  //purity ranking
			if(dijet.M()>=500&&fabs(jet[1].Eta()-jet[0].Eta()) >= 3.5&&fabs(dijet.DeltaPhi(HZgVar.vec_higgs)) >= 2.4&&fabs(Zeppen) <= 2.5)HZgVar.jets.push_back(jetID[j]);
			else if(dijet.M()>=500&&fabs(jet[1].Eta()-jet[0].Eta()) >= 3.5&&fabs(dijet.DeltaPhi(HZgVar.vec_higgs)) >= 2.4)HZgVar.jets.push_back(jetID[j]);
...
bool leptag(TreeReader &data, Int_t channel, vector<int> &muID, vector<float> &muPt, HZgVariables &HZgVar)
{
  ...
  AddEle16Selections(data,98, eleID, HZgVar.lepPt[0],HZgVar.lepPt[1],channel);
  TLorentzVector alep;
  if(channel==0)
    {
      if(eleID.size()>0)
