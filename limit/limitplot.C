...
void limitplot(TString fin, TString type, TString cat){
  ...
  for (int i = 0; i < 11; i++){
    ...
    TString f1name = "DataCards/higgsCombine" + fin + "_" + type + "_" + cat +"_nofix.Asymptotic.mH" + name[i] + ".root" ;//remove
    TFile *f = new TFile(fname);
    TFile *f1 = new TFile(f1name);//rm
    TTree *t = (TTree*)f->Get("limit");
    TTree *t1 = (TTree*)f1->Get("limit");//rm
    float tmp2l,tmp1l,tmpm,tmp1u,tmp2u;
    Double_t limit;
    Float_t quantileExpected;
    t->SetBranchAddress("limit", &limit);    
    t->SetBranchAddress("quantileExpected", &quantileExpected);
    t1->SetBranchAddress("limit", &limit);
    t1->SetBranchAddress("quantileExpected", &quantileExpected);
	...
	{
	  t1->GetEntry(i);
	  if (i == 0) lower2s.push_back((limit-tmp2l)/limit);
	  if (i == 1) lower1s.push_back((limit-tmp1l)/limit);
	  if (i == 2) medlimit.push_back((limit-tmpm)/limit);
	  if (i == 3) upper1s.push_back((limit-tmp1u)/limit);
	  if (i == 4) upper2s.push_back((limit-tmp2u)/limit);
	  if(i==2)cout<<limit<<" "<<tmpm<<endl;
	}
  ...
  c->RedrawAxis();
  c->GetFrame()->Draw();
  TString pname  = "plot/limit_" + fin + "_" + type + "_" + cat + "compare.png";
  c->Print(pname);
