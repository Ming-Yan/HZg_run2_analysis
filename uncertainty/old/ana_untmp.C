  ...
  for (Long64_t ev = 0; ev <  data.GetEntriesFast(); ev++)
    {
      ...
      if(un==0)eleScale = data.GetPtrFloat("eleScale_stat_up");
      else if(un==1)eleScale = data.GetPtrFloat("eleScale_stat_dn");
      else if(un==2)eleScale = data.GetPtrFloat("eleScale_syst_up");
      else if(un==3)eleScale = data.GetPtrFloat("eleScale_syst_dn");
      else if(un==4)eleScale = data.GetPtrFloat("eleScale_gain_up");
      else if(un==5)eleScale = data.GetPtrFloat("eleScale_gain_dn");
      if(un==6) eleRes = data.GetPtrFloat("eleResol_rho_up");
      else if(un==7) eleRes = data.GetPtrFloat("eleResol_rho_dn");
      else if(un==8) eleRes = data.GetPtrFloat("eleResol_phi_up");
      else if(un==9) eleRes = data.GetPtrFloat("eleResol_phi_dn");
     vector <float> elePt;elePt.clear();
      for (unsigned int i = 0 ; i < nEle; i++)
	{
	  elePt.push_back(elePt_[i]);
	  /*if(un<6)elePt.push_back(elePt_[i]*eleScale[i]);
	    else elePt.push_back(elePt_[i]*rgen->Gaus(1.,eleRes[i]));*/
	}
      ...
      if(mucorr == true)
	{
	  ...
	  for (Int_t i = 0; i < nMu ; i++)
		...
		for (unsigned int mc = 0; mc < imcMu.size(); mc++){
		 ...
		 SF = rc.kSmearMC(muCharge[i], muPt_[i], muEta[i], muPhi[i], muTrkLayers[i], rand1,  0, 0);
    ...
    //if(wei_passmu==0)cout<<HZgvar.SFmutrg_[0]<<HZgvar.SFmutrg_[1]<<" "<<HZgvar.SFmuID_[0]<<" "<<HZgvar.SFmuID_[1]<<endl;
      ...
      if(un==0)phoScale = data.GetPtrFloat("phoScale_stat_up");
      else if(un==1)phoScale = data.GetPtrFloat("phoScale_stat_dn");
      else if(un==2)phoScale = data.GetPtrFloat("phoScale_syst_up");
      else if(un==3)phoScale = data.GetPtrFloat("phoScale_syst_dn");
      else if(un==4)phoScale = data.GetPtrFloat("phoScale_gain_up");
      else if(un==5)phoScale = data.GetPtrFloat("phoScale_gain_dn");
      if(un==6) phoRes = data.GetPtrFloat("phoResol_rho_up");
      else if(un==7) phoRes = data.GetPtrFloat("phoResol_rho_dn");
      else if(un==8) phoRes = data.GetPtrFloat("phoResol_phi_up");
      else if(un==9) phoRes = data.GetPtrFloat("phoResol_phi_dn");
      ...
      for(int i = 0; i < nJet ; i++)
	{
      ...
      for (int l = 0 ; l < nMC; l++)
	{
	  if(mcPID[l]==22&&(fabs(mcMomPID[l])==11||fabs(mcMomPID[l])==13||mcMomPID[l]==23)&&(((mcStatusFlag[l] >> 1) & 1)\
											     ==1||((mcStatusFlag[l] >> 0) & 1)==1))fsrphoton.SetPtEtaPhiM(mcPt[l],mcEta[l],mcPhi[l],mcMass[l]);
	}
      ...
      if(isrefit)
	{
	  ...
	  //{
	      ...
	      refitlep = kinZfitter->GetRefitP4s();
            ...
            float new_pt1 = (un<6)? refitlep[0].Pt()*eleScale[lepID_kin[0]]: refitlep[0].Pt()*rgen->Gaus(1.,eleRes[lepID_kin[0]]);
            float new_pt2 =(un<6)? refitlep[1].Pt()*eleScale[lepID_kin[1]]: refitlep[1].Pt()*rgen->Gaus(1.,eleRes[lepID_kin[1]]);
          
             refitlep[0].SetPtEtaPhiM(new_pt1,lep[0].Eta(),lep[0].Phi(),0.511*0.001);
	      refitlep[1].SetPtEtaPhiM(new_pt2,lep[1].Eta(),lep[1].Phi(),0.511*0.001);
