...
//void mana(Int_t channel = 0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, Int_t processID=0)
{
  ...
  for (Long64_t ev = 0; ev <  data.GetEntriesFast(); ev++)
    ...
    //if(wei_passmu==0)cout<<HZgvar.SFmutrg_[0]<<HZgvar.SFmutrg_[1]<<" "<<HZgvar.SFmuID_[0]<<" "<<HZgvar.SFmuID_[1]<<endl;
      ...
      float* phoEt_    = data.GetPtrFloat("phoEt");//use this for pho uncertainties
      float* phoScale = NULL;
      float* phoRes = NULL;
	...
	{
	  if(un<6)phoEt.push_back(phoEt_[i]*phoScale[i]);                                                                             
	  else phoEt.push_back(phoEt_[i]*rgen->Gaus(1.,phoRes[i]));
      ...
      if(isrefit)
	{
	  ...
	  //{
	      ...
	      // HZgvar.mllerr = kinZfitter->GetMZ1Err();
