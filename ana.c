...
//#include "plugin/untuplizer.h"//--for data in eos
#include "plugin/ElectronSelections1.h"
#include "plugin/MuonSelections1.h"//for mucalibpt
//#include "plugin/MuonSelections2.h"//for mupt
...
void mana2(const char* inpath, TString outpath = "minitree.root", Int_t channel=0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, int isGH = 0)
//void mana(Int_t channel = 0, Float_t xs=1, Float_t lumi=1, Int_t aMCatNLO=0, Int_t photonRemoval=0, Int_t processID=0)
{
  ...
  TFile *fe_trg2 = new TFile("external/SFs_Leg2_Ele12_HZZSelection_Tag35.root");
  ...
  */
  TH2F              *hm_ZZSF = (TH2F*) fm_HZZ->Get("FINAL");
  TH2F              *hm_ZZSFErr = (TH2F*) fm_HZZ->Get("ERROR");
  ...
  //prepare tree branch
  Int_t run_, lumis_, nVtx_, lepCh_[2],nMC_;
  Long64_t event_;
  Int_t  cat, totalev_, alepCh, totmatch;
  Float_t puwei_= 1.; Float_t puweiUp_ = 1.; Float_t puweiDown_ = 1.; Float_t mcwei_ = 1.; Float_t genwei_ = 1.; Float_t totwei_ = 1.;
  Float_t lepPt_[2], lepEta_[2], lepPhi_[2], lepSCEta_[2],lepSCPhi_[2], lepR9_[2], lepPhoMVA_[2], mll_;//lepton related variables
  Float_t gPt_, gEta_, gPhi_, gSCEta_, gR9_, gMVA_, gSSMVA_;//photon related variables
  Float_t dRlg_, mllg_; //three body variables
  Float_t alepPt,alepEta,alepPhi, alepSCEta, alepR9, // lepton tag variables
    bjetPt, bjetEta, bjetPhi, bjetEn,bjetphodR, bCsv2,//b-tag variables
    VBFPt[2], VBFEta[2],VBFPhi[2], VBFEn[2],Zeppen;// VBF tag variables
  Float_t mcmllg,mcmll;
  Int_t isLep, isVBF, isboost, isbtag;//use to tag the category--NO STORED IN TREE!!!!
  ...
  //set initial value
  unGSF_[0] = unGSF_[1] = unGSF_[2] = uneID_[0] = uneID_[1] = uneID_[2] = unetrg_[0] = unetrg_[1] = unedz_[0] = unedz_[1] = 
    unmutrg_[0] = unmutrg_[1] = unmuTrk_[0] = unmuTrk_[1] = unmuTrk_[2] = unmuID_[0]= unmuID_[1] = unmuID_[2] = unmuIso_[0] = unmuIso_[1] = unmuIso_[2] = 0;
  ...
  outtreeZg_->Branch("totwei",    &totwei_,    "totwei/F");
  ...
  outtreeZg_->Branch("mcmllg",    &mcmllg,    "mcmllg/F");
  outtreeZg_->Branch("mcmll",     &mcmll,     "mcmll/F");
  //-----------------------------------------------------//
  //tag variables
  ...
  cout<<"mcwei:"<<mcwei_<<"   xsection:"<<"  totalev:"<<totalEvents<<endl;
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++)
    {
      ...
      //kinematic fit var
      Int_t nDiLep       = data.GetInt("nDiLep");
      Int_t* diLepFlavor = data.GetPtrInt("diLepFlavor");
      Int_t* diLepKFVtxIsValid = data.GetPtrInt("diLepKFVtxIsValid");
      Int_t* diLep1Index = data.GetPtrInt("diLep1Index");
      Int_t* diLep2Index = data.GetPtrInt("diLep2Index");
      float* diLep1KFPx  = data.GetPtrFloat("diLep1KFPx");
      float* diLep1KFPy  = data.GetPtrFloat("diLep1KFPy");
      float* diLep1KFPz  = data.GetPtrFloat("diLep1KFPz");
      float* diLep2KFPx  = data.GetPtrFloat("diLep2KFPx");
      float* diLep2KFPy  = data.GetPtrFloat("diLep2KFPy");
      float* diLep2KFPz  = data.GetPtrFloat("diLep2KFPz");
	...
	mucorrPt.push_back(muPt[i]);
      ...
      //TightMuons2015(data, muID, Zmm, mucorrPt, channel); // for no skim
      //TightMuons2015(data, muID, Zmm, channel);

	HZZmuIDforZg(data, muID, Zmm, channel);
	//HZZmuIDforZg(data, muID, Zmm, mucorrPt, channel);
	  ...
	  for (Int_t j = 0 ; j < nMC; ++j)
	      ...
	      if (fabs(mcMomPID[j]) != 23) continue;
	  ...
	  for (Int_t j=0; j< 2; ++j)
	      ...
	      for(int i = 0; i<nDiLep;i++)
		{
		  if(diLepKFVtxIsValid[i]==0)break;
		  if(fabs(diLepFlavor[i])==13)continue;
		  float lepen;
		  if(diLep1Index[i]==ZeeID[j])
		    {
		      lepen = sqrt(pow(diLep1KFPx[i],2)+pow(diLep1KFPy[i],2)+pow(diLep1KFPz[i],2)+pow(0.511*0.001,2));
		      lep[j].SetPxPyPzE(diLep1KFPx[i],diLep1KFPy[i],diLep1KFPz[i],lepen);
		    }
		  else
		    {
		      lepen = sqrt(pow(diLep2KFPx[i],2)+pow(diLep2KFPy[i],2)+pow(diLep2KFPz[i],2)+pow(0.511*0.001,2));
		      lep[j].SetPxPyPzE(diLep2KFPx[i],diLep2KFPy[i],diLep2KFPz[i],lepen);
		    }
		}    
	      lepEta_[j] = lep[j].Eta();
	      lepPt_[j] = lep[j].Pt();
	  ...
	  for (Int_t j=0; j< 2; ++j)
            {
	      lepCh_[j] = muCharge[Zmm[j]];
	      lep[j].SetPtEtaPhiM(muPt[Zmm[j]], muEta[Zmm[j]], muPhi[Zmm[j]], 105.7*0.001);
	      for(int i = 0; i<nDiLep;i++)
                {
                  if(diLepKFVtxIsValid[i]==0)break;
                  if(fabs(diLepFlavor[i])==11)continue;
                  float lepen;
                  if(diLep1Index[i]==Zmm[j])
                    {
                      lepen = sqrt(pow(diLep1KFPx[i],2)+pow(diLep1KFPy[i],2)+pow(diLep1KFPz[i],2)+pow(0.511*0.001,2));
                      lep[j].SetPxPyPzE(diLep1KFPx[i],diLep1KFPy[i],diLep1KFPz[i],lepen);
                    }
	...
	mcmllg = mcllg.M();
      ...
      phoIndex = phoID[0];
      pho.SetPtEtaPhiM(phoEt[phoID[0]], phoEta[phoID[0]], phoPhi[phoID[0]], 0.);
      if (data.HasMC() && photonRemoval == 1)
	{
	  for (Int_t i=0; i<nMC; ++i)
	    {
      ...
      if(fabs(phoEta[phoID[0]])<1.479)phoR9[phoID[0]] = gg_R9EB->Eval(phoR9[phoID[0]]);
      else phoR9[phoID[0]] = gg_R9EE->Eval(phoR9[phoID[0]]);
      if (pho.DeltaR(lep[0]) < 0.4 || pho.DeltaR(lep[1]) < 0.4) continue;
      N_mllgmll++;//double counting removal and dR
      higgs = zll + pho;
      if (higgs.M() + zll.M() < 185.) continue;
      ...
      for(int i = 0 ; i <nJet ; i++)
	{
	  ...
	  if(bjet.DeltaR(pho) < 0.4) continue;
	  if(bjet.DeltaR(lep[0])<0.4||bjet.DeltaR(lep[0])<0.4) continue;
	  bjetPt = jetPt[i];
	  bjetEn = tmpjetEn[i];
	  bjetPhi = jetPhi[i];
	  bjetEta = jetEta[i];
	  if (jetBtag[i]>=0.8484&&jetBtag[i]<0.935) isbtag = 0;//medium
	  else if (jetBtag[i]>=0.935) isbtag =1;//tight
	  cat = 11;
	  bCsv2 = jetBtag[i];
	}	
      ...
      if(isVBF!=1&&isLep!=1)
	{
      ...
      //if(isbtag!=-99)cout<<"acc*eff"<<isbtag<<endl;
      if(lepmatch[1]==1&&lepmatch[0]==1&&phomatch==1) totmatch = 1;
	  ...
	  if(cat<6||cat>9) sfGSF_[2] = sfeID_[2]  = sfmuID_[2] = sfmuIso_[2] = sfmuTrk_[2] = 1.; //set the SFs for no lepton tag cat
		  ...
		  sfmuID_[2] = hm_ZZSF->GetBinContent(hm_ZZSF->FindBin(alepEta,tmpMuPt));
		  unmuID_[2] = hm_ZZSFErr->GetBinContent(hm_ZZSFErr->FindBin(alepEta,tmpMuPt));
		  /*if (alepPt >= 20)
                    {
	  ...
	  if(totSF_>1.7)cout<<sfGSF_[0]<<" "<<sfGSF_[1]<<" "<<sfGSF_[2]<<" "<<sfedz_[0]<<" "<<sfedz_[1]<<" "<<sfetrg_[0]<<" "<<sfetrg_[1]<<" "<<sfeID_[0]<<" "<<sfeID_[1]<<" "<<sfeID_[2]<<" "<<sfgVeto_<<" "<<sfgID_<<" "<<sfmuTrk_[2]<<" "<<sfmuID_[2]<<" "<<sfmuIso_[2]<<endl;
        }//ele SFs end
	  ...
	  for(Int_t i = 0 ; i <2 ; i++)
	      ...
	      else 
		{
	      ...
	      sfmuID_[i] = hm_ZZSF->GetBinContent(hm_ZZSF->FindBin(lepEta_[i],tmpMuPt));
	      unmuID_[i] = hm_ZZSFErr->GetBinContent(hm_ZZSFErr->FindBin(lepEta_[i],tmpMuPt));		  
	      /*if(tmpMuPt>20)
                {
	  ...
	  if(totSF_ > 1.7||totSF_==0)cout<</*sfmuTrk_[0]<<" "<<sfmuTrk_[1]<<" "<<sfmuTrk_[2]<<" "<<*/sfmuID_[0]<<" "<<sfmuID_[1]<<" "<<sfmuID_[2]<<" "/*<<sfmuIso_[0]<<" "<<sfmuIso_[1]<<" "<<sfmuIso_[2]<<" "*/<<sfgVeto_<<" "<<sfgID_<<" "<<sfGSF_[2]<<" "<<sfeID_[2]<<endl;
	  totwei_ = puwei_*genwei_*mcwei_*totSF_;
        }//muon channel SFs end

      //if(isbtag!=-99)cout<<"SFs"<<isbtag<<endl;
